...
public class SearchMaze {
    ...
    private static boolean searchMazeHelper(List<List<Color>> maze, List<Coordinate> path, Coordinate s, Coordinate e) {
        if (isValidPos(s, maze.size(), maze.get(0).size()) && maze.get(s.x).get(s.y) == Color.WHITE) {
            path.add(s);
            if (s.equals(e)) {
                return true;
            }
            // this is as set this point as visited
            maze.get(s.x).set(s.y, Color.BLACK);
            if (searchMazeHelper(maze, path, new Coordinate(s.x + 1, s.y), e) ||
                    searchMazeHelper(maze, path, new Coordinate(s.x - 1, s.y), e) ||
                    searchMazeHelper(maze, path, new Coordinate(s.x, s.y + 1), e) ||
                    searchMazeHelper(maze, path, new Coordinate(s.x, s.y - 1), e)) {
                return true;
            }
    ...
    private static boolean isValidPos(Coordinate s, int M, int N) {
        return s.x >= 0 && s.x < M && s.y >= 0 && s.y < N;
    }
